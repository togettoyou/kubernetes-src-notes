...
**DRA（Dynamic Resource Allocation，动态资源分配）** 是 Kubernetes 为此设计的下一代方案，v1.34 升级为 GA。它引入了一套新的 API：
...
- 拦截 `_nvmlDeviceGetMemoryInfo`：使 `nvidia-smi` 只显示 `CUDA_DEVICE_MEMORY_LIMIT_<index>` 设定的配额值，而非物理总显存
