...
> **注**：Device Plugin 只能向调度器上报资源数量（整数计数），调度器无法感知设备的具体属性（显存大小、型号、拓扑等）。
>
> Kubernetes v1.34 已将 **DRA（Dynamic Resource Allocation，动态资源分配）** 升级为 GA，通过 `ResourceClaim` /
`DeviceClass` /
`ResourceSlice` 等 API 提供属性驱动的细粒度设备分配能力，是 Device Plugin 的下一代替代方案。
>
> 当前主流工具链（GPU Operator、HAMI）仍基于 Device Plugin，DRA 生态尚在追赶中。

### 1.2 扩展资源声明方式

用户在 Pod Spec 中声明 GPU 需求：
...
对 Kubernetes 调度器而言，`nvidia.com/gpu` 与 CPU/Memory 一样是一种可量化资源，调度逻辑一致，**差异在于 Device Plugin
内部如何映射到物理设备**。

### 1.3 GPU 共享的挑战

标准 Device Plugin 模式下，每块物理 GPU 只能分配给一个 Pod（`nvidia.com/gpu: 1` 意味着独占整张卡）。这带来两个问题：

- **利用率低**：推理服务通常只用到 GPU 20%~40% 的算力和显存，其余资源闲置
- **灵活性差**：无法为不同业务按需分配显存/算力配额

为解决上述问题，衍生出了 **时间切片（Time-Slicing）**、**MIG 硬件分区**、**vGPU 软件虚拟化** 等多种 GPU 共享技术。
...
HAMI（Heterogeneous AI Computing Virtualization Middleware，异构 AI 计算虚拟化中间件）是一个 **CNCF Sandbox 项目**，前身为
`k8s-vGPU-scheduler`。

HAMI 的核心定位是在 **不修改 NVIDIA 驱动、不改造应用代码** 的前提下，通过 CUDA API 劫持技术实现 GPU 资源的虚拟化和细粒度隔离，使多个
Pod 能安全地共享同一块物理 GPU。
...
Pod 调度到目标节点后，Kubelet 调用 Device Plugin 的 `Allocate` 接口，Device Plugin 在响应中完成三件事：
...
- 拦截 `_nvmlDeviceGetMemoryInfo`：使 `nvidia-smi` 只显示 `CUDA_DEVICE_MEMORY_LIMIT_X` 设定的配额值，而非物理总显存
- 拦截 `cuMemAlloc_v2` / `cuMemoryAllocate` 等内存分配函数：分配前执行 OOM 检查——若当前 Pod 已用显存 + 本次申请量 >
  `CUDA_DEVICE_MEMORY_LIMIT_X`，直接返回 `CUDA_ERROR_OUT_OF_MEMORY`，阻止超额分配
...
#### 2.5.2 节点调度策略（Node-Scheduler-Policy）
...
```
节点1 得分 = ((1 + 3) / 4) × 10 = 10
节点2 得分 = ((1 + 2) / 4) × 10 = 7.5  ← 得分低，Spread 选低分节点
```

调度结果：pod1 → 节点1，pod2 → 节点2（分散到不同节点）
...
#### 2.5.3 GPU 卡调度策略（GPU-Scheduler-Policy）
...
```
GPU 得分 = ((申请 core + 已用 core) / 总 core + (申请 mem + 已用 mem) / 总 mem) × 10
```
