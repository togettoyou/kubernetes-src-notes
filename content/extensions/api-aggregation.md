...
participant "管理员" as admin #EDE7F6
participant "AggregatorServer" as agg #BBDEFB
participant "Extension API Server" as aa #FFE0B2
participant "kubectl" as client #C8E6C9

== 注册阶段 ==
admin -> agg : 创建 APIService
agg -> aa : GET /apis
aa --> agg : APIGroupDiscoveryList
agg -> agg : 合并到全局 Discovery 缓存

== 运行阶段 ==
client -> agg : GET /apis（Discovery 请求）
agg --> client : 直接返回缓存，不回调 AA 服务

client -> agg : GET /apis/simple.aa.io/v1beta1/...（业务请求）
agg -> aa : 转发
aa --> agg : 资源数据
agg --> client : 资源数据
@enduml
...
1.27 之前的 kube-apiserver 通过三次请求完成 Discovery：先请求 `/apis` 获取 `APIGroupList`，再请求 `/apis/<group>` 获取 `APIGroup`，最后请求 `/apis/<group>/<version>` 获取 `APIResourceList`。
...
client -> api : GET /apis
api --> client : APIGroupList
note right #FFF9C4 : 有哪些 Group

client -> api : GET /apis/simple.aa.io
api --> client : APIGroup
note right #FFF9C4 : 该 Group 有哪些版本

client -> api : GET /apis/simple.aa.io/v1beta1
api --> client : APIResourceList
note right #FFF9C4 : 该版本有哪些资源
...
三个对象需要分别实现（`pkg/apis/discovery.go`）：
...
client -> api : GET /apis
api --> client : APIGroupDiscoveryList
note right #FFF9C4 : Group + Version + Resource\n一次全部返回
