...
participant "DRA 驱动\n(Controller)" as ctrl #FFE0B2
participant "kube-apiserver" as api #E3F2FD
participant "kube-scheduler" as sch #E8F5E9
participant "kubelet" as k #BBDEFB
participant "DRA 驱动\n(kubelet Plugin)" as plug #FFE0B2
participant "容器运行时\n(containerd)" as cri #F3E5F5
actor "用户" as user

== 驱动启动 ==
ctrl -> api : 创建 ResourceSlice\n（发布节点上的设备及属性）
plug -> plug : 在 plugins/<driver>/ 启动 dra.sock
plug -> plug : 在 plugins_registry/ 创建注册 socket
k -> plug : GetInfo()\n检测到注册 socket，发起握手
plug --> k : 驱动类型 = DRAPlugin\ndra.sock 路径
k -> plug : NotifyRegistrationStatus(registered=true)

== 调度阶段 ==
user -> api : 创建 ResourceClaim\n（声明需要的设备属性）
user -> api : 创建 Pod\n（引用 ResourceClaim）
sch -> api : 读取 ResourceSlice\n对 attributes 跑 CEL 匹配
sch -> api : 写入 ResourceClaim.status.allocation\n（记录分配结果：哪个节点、哪个设备）
sch -> api : 写入 Pod.spec.nodeName

== 节点准备阶段 ==
k -> plug : NodePrepareResources(claims)
plug -> plug : 读取 ResourceClaim.status.allocation\n初始化设备
plug --> k : 返回 CDI 设备 ID 列表
k -> cri : 创建容器\n（携带 CDI 设备 ID）
