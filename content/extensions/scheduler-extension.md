...
调度 framework 中的 PreEnqueue、QueueSort、PreFilter、PostFilter、PreScore、Score、Reserve、Permit、PreBind、PostBind 等扩展点，extender 均无法介入。这也是 extender 最大的限制。
...
extenders:
  - urlPrefix: "http://localhost:8080"  # extender 服务地址
    ...
    nodeCacheCapable: false             # true 时调度器只传节点名（extender 需自行维护节点缓存）；false 时传完整 Node 对象
...
- **阶段受限**：只能介入 Filter、Prioritize、Preempt、Bind 四个阶段，无法介入 PreEnqueue（Pod 入队门控）、QueueSort（影响 Pod 入队顺序）、Reserve（资源预留）、Permit（调度批准）等其他阶段
